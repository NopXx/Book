...
function Profile({ uData, isLogin }) {
  ...
  const getData = async() => {
    await axios.get(`http://localhost:3001/user/${uid.u_id}`).then((response) => {
      setUserData(response.data);
    });
    await axios.get(`http://localhost:3001/history/${uid.u_id}`).then((response) => {
      setHistoryNum(response.data);
    });
    await axios
      .post(`http://localhost:3001/lend/admin/data`, {
        sql: `SELECT * FROM lends WHERE u_id = ${uid.u_id} AND Date_night IS NOT NULL`,
      })
      .then((response) => {
      ...
      await axios
      .post(`http://localhost:3001/lend/admin/data`, {
        sql: `SELECT * FROM lends WHERE u_id = ${uid.u_id} AND Date_night IS NULL`,
      })
      .then((response) => {
      ...
      await axios
      .post("http://localhost:3001/lend/admin/data", {
        sql: `SELECT lends.id, lends.b_id, books.b_name, lends.u_id, user.u_fname, lends.s_id, staff.s_fname, lends.Borrow_date, lends.Time_allowed, lends.Date_night FROM (((lends INNER JOIN user ON lends.u_id = user.u_id) LEFT OUTER JOIN staff ON lends.s_id = staff.s_id)INNER JOIN books ON lends.b_id = books.b_id ) WHERE lends.u_id = ${uid.u_id} ORDER BY lends.Borrow_date desc LIMIT 5`,
      })
      .then((response) => {
  ...
  // const getUserData = () => {
  //   axios.get(`http://localhost:3001/user/${uid.u_id}`).then((response) => {
  //     setUserData(response.data);
  //   });
  // };
  useEffect(() => {
        ...
        <div class="grid flex-grow card bg-base-100 shadow-lg rounded-lg lg:w-1/3">
          <div class="avatar placeholder">
            <div class="bg-primary text-primary-content max-w-lg rounded-lg shadow-2xl w-28 md:w-60">
              <span class="text-3xl">{usData.u_fname}</span>
            </div>
          </div>
          {message1 === "" ? (
            ""
              ...
              >
                {isActive === false ? (
                  <div className="justify-start p-1 m-2">
                    <p className="py-1">ชื่อ : {usData.u_fname}</p>
                    <p className="py-1">นามสกุล : {usData.u_lname}</p>
                    <p className="py-1">Username : {usData.u_user}</p>
                    <p className="py-1">เบอร์โทรศัพท์ : {usData.u_tel}</p>
                  </div>
                ) : (
                  <>
